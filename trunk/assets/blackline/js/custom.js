...
//Project Notes
$('.ajax-silent #send').live("click", function(e){
...
//Clickable rows
    ...
    $("table#custom_quotations_requests td").live("click", function(){
      var id = $(this).parent().attr("id");
      if(id){
        var site = $(this).closest('table').attr("rel");
        if(!$(this).hasClass('option')){window.location = "cview/"+id;}
      }
    });
    $("table#quotation_form td").live("click", function(){
      var id = $(this).parent().attr("id");
      if(id){
        var site = $(this).closest('table').attr("rel");
